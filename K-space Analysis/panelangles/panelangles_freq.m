...
function [directions,sx,sy,sz] = panelangles_freq(panellist, probelist, point, samp, sampconv, freqscal)
...
pick=floor(.2/sampconv)*numel(freqscal);

for curfreq=freqscal
    for n1=1:r1
        for n2=1:r2
            for x1=datasample(panellist(n1,1):sampconv:panellist(n1,3),...
                    floor(numel(panellist(n1,1):sampconv:panellist(n1,3))/numel(freqscal)))
                for y1=datasample(panellist(n1,2):sampconv:panellist(n1,4),...
                        floor(numel(panellist(n1,2):sampconv:panellist(n1,4))/numel(freqscal)))
                    for x2=datasample(probelist(n2,1):sampconv:probelist(n2,3),...
                            floor(numel(probelist(n2,1):sampconv:probelist(n2,3))/numel(freqscal)))
                        for y2=datasample(probelist(n2,2):sampconv:probelist(n2,4),...
                              floor(numel(probelist(n2,2):sampconv:probelist(n2,4))/numel(freqscal)))  
