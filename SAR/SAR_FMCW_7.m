...
nXMeas = ceil(Lx/(xResEst)); %@ double res
nYMeas = ceil(Ly/(yResEst));
...
target = targetAsymSmiley(2*floor(xResEst/dx)*dx,2*floor(yResEst/dy)*dy,xgrid,ygrid); %weird asymmetric target at resolution
%make 
