...
% sb_kr_xn = ifftshift(fft(fftshift(sb.*exp(-1j*wc.*ts.'),1),[],1)); %!!!!!!!!!!!!!!!!!!!!!!
% sb_kr_xn = fft(sb.*exp(-1j*wc.*ts.'),[],1); %!!!!!!!!!!!!!!!!!!!!!!
sb_kr_xn = ifftshift(fft(sb,[],1),1); %!!!!!!!!!!!!!!!!!!!!!!

% sb_kr_xn = fft(sb,[],1); %!!!!!!!!!!!!!!!!!!!!!!

% sb_kr_xn = sb.*exp(-1j*wc.*ts.');
% sb_kr_xn = sb.*exp(-1j*Krc.*(c*ts.')); %!!!!!!!!!!!!!!!!!!!!!!
...
%         sb_kx_ky(:,jj) = interp1(Ky(indx_vec,jj), sb_kr_kxn_mf(indx_vec,jj), Ky_resample, 'nearest',0);
%     end
% end
...
% indx_vec2 = zeros(size(Ky,1),1);
% for jj = 1:size(sb_kr_kxn_mf,2)
%     indx_vec1 = squeeze(squeeze(real(Ky(1:end/2,jj))~=0));
%     indx_vec2(size(Ky,2)/2:size(Ky,2)) = squeeze(squeeze(real(Ky(end/2+1:end,jj))~=0));
%     
%     if ~(sum(indx_vec1)==0 || sum(indx_vec1)==1) %check that there are enough points to interpolate
%         sb_kx_ky(1:end/2,jj) = interp1(Ky(indx_vec1,jj), sb_kr_kxn_mf(indx_vec1,jj), flip(Ky_resample(end/2+1:end)), 'linear',0);
%     end
%     if ~(sum(indx_vec2)==0 || sum(indx_vec2)==1) %check that there are enough points to interpolate
%         sb_kx_ky(end/2+1:end,jj) = interp1(Ky(indx_vec2,jj), sb_kr_kxn_mf(indx_vec2,jj), Ky_resample(end/2+1:end), 'linear',0);
%     end
% end
