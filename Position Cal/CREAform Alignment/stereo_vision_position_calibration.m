%%
close all
data_panel=load('C:\Users\Jonah Gollub\Downloads\StereroVision_Panels.mat\duke_system_fiducials.mat')
...
%account for displacment of scanning fiducial
%translate y
panelcenter(:,2)=panelcenter(:,2)-5.5;

%find B3 position for ref origin
...
%traslate z
panelcenter(:,3)=panelcenter(:,3)-panelcenter(7,3);

%account for NFS plane position
panelcenter(:,3)=panelcenter(:,3)+86;
...
normal=fitNormal(panelcenter,1);

% [alpha==z, beta==y, gamma==x]
for i=1:12
    txA(i,1)=atan2(normal(2),normal(1)); % Z Rotation
    txB(i,1)=-atan2(normal(3),normal(1))-0.7*pi/180; % Y Rotation. nfs compensation
    txC(i,1)=0;                          % X Rotation
    txX(i,1)=panelcenter(i,1);           % X Position
    txY(i,1)=panelcenter(i,2);           % Y Position
    txZ(i,1)=panelcenter(i,3);           % Z Position
end

T=transform(txA(1),txB(1),txC(1),0,0,0);
...
new_anntenna_pos.z0=new_anntenna_pos.z0-panel_origin(3);

% NFS plane
new_anntenna_pos.z0=new_anntenna_pos.z0+55;
...
normal=fitNormal(final_anntenna_pos,1);

for i=1:24
    rxA(i,1)=atan2(normal(2),normal(1)); % Z Rotation
    rxB(i,1)=-atan2(normal(3),normal(1)); % Y Rotation. nfs compensation
    rxC(i,1)=0;                          % X Rotation
    rxX(i,1)=final_anntenna_pos(i,1);    % X Position
    rxY(i,1)=final_anntenna_pos(i,2);    % Y Position
    rxZ(i,1)=final_anntenna_pos(i,3);    % Z Position
end
...
save('pose.mat','txA','txB','txC','txX','txY','txZ','rxA','rxB','rxC','rxX','rxY','rxZ');
