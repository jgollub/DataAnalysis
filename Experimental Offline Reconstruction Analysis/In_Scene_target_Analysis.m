...
choosePanels=zeros(1,12);
choosePanels(panel_set)=1;
%%probe=     [P1, P2, P3, p4, p5, p6];
% chooseProbes=[ 1, 1,  1,  1,  1,  1];
chooseProbes=zeros(1,6);
chooseProbes(probe_set)=1;
...
%%Plotting
 ...
 if isempty(f_est_plot)
            ...
            try figure(image_plot)
             ...
             subplot(numel(choosePanels),numel(chooseProbes),...
     find(choosePanels)+numel(choosePanels)*(find(chooseProbes)-1))
