...
/* Implements multiplication with A and AT using the definition of sparse structures as in Matlab 
The Atype is a struct holding the values Av, and the corresponding rows Ar. The array Ac hold the number of non-zero elements as the difference between two indexes in Ac */
void Amul(Atype A, double *x,double *y){
	register int j,i,jc,ic,nnzc,ki,kj,ii,jj,m,n,mm,nn;
	double sum;

	if(A.sparseOrPSF==0){ /* defines a sparse matrix*/
		j = 0;
		i = 0;
		...
		/*printf("ID4 Amul %d %d %d %d\n",A.Anr,A.Anc,A.PSFnr,A.PSFnc);DRAW;*/
		for(i=0;i<A.xu;i++){
			for(j=0;j<A.xv;j++){
				sum=0;
				m = -1;
	...
	/*printf("ID4 Amul %d %d %d %d %d %d\n",A.Anr,A.Anc,A.PSFnr,A.PSFnc,A.xu,A.xv);DRAW;*/
}
